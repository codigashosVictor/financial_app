...
from app.core.ownership import get_owned_card, get_owned_subscription
...
async def sub_create(
    ...
    supabase = get_supabase(user["access_token"])
    get_owned_card(supabase, user["id"], card_id, "id", active_only=True)
...
async def sub_toggle(request: Request, sub_id: str, _csrf: None = Depends(verify_csrf)):
    user = require_user(request)
    if not user:
        return RedirectResponse("/login", status_code=302)

    supabase = get_supabase(user["access_token"])
    current = get_owned_subscription(supabase, user["id"], sub_id, "is_active")
...
async def sub_delete(request: Request, sub_id: str, _csrf: None = Depends(verify_csrf)):
    user = require_user(request)
    if not user:
        return RedirectResponse("/login", status_code=302)
